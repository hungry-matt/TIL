...
- 이렇게 컴파일 시에 미리 타입 검사를 수행 하면 `객체 타입의 안정성을 높이고` 반환값에 대한 `타입 변환 및 타입 검사`에 들어가는 `노력을 줄일 수 있다`.

# Vector vs ArrayList
`크기가 동적인 배열`을 다룰때 사용하는 클래스 이다.

- Vector
    - 동기화된 상태이다.(Thread-safe)
    - 상대적으로 속도가 느리다.(동기화 되어 있기 때문)
- ArrayList
    - 동기화가 안된 상태이다.
    - 상대적으로 속도가 빠르다.(동기화가 안되어있기 때문)
    - 멀티스레드 사용 환경이 아닐 경우 훨씬 빠르다.

# Thread-safe
스레드 안전이란 `멀티 스레드 구현시` 여러 스레드가 `하나의 자원에 동시 접근`이 이루어져도 `프로그램 실행에 문제가 없음`을 뜻한다.

# ArrayList vs LinkedList
두 클래스는 모두 Java에서 제공하는 List 인터페이스를 구현하고 있는 Collection 구현체이다.

- ArrayList
    - 내부적으로 `데이터를 배열로 관리`하고 `데이터 추가/삭제 시 임시 배열을 생성`해 데이터를 복사 한다.
    - `데이터별 인덱스`가 있어 `검색에 유리` 하다.
    - `임시 배열을 사용`하기 때문에 `데이터 추가/삭제`의 경우에는 `불리` 하다.

- LinkedList
    - 내부적으로 `노드 단위로 데이터를 관리` 한다. 자신의 앞, 뒤 노드 정보만 가지고 있다.
    - `인덱스가 없기 때문에` 검색 시 `전 노드를 순회`해야하여 `검색시 불리` 하다.
    - 데이터 추가/삭제 시 `불필요한 데이터 복사가 없어 유리` 하다.
